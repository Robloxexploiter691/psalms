...
do
	...
	-- // Extension
	Psalms.__index = Psalms
	Psalms.Pages.__index = Psalms.Pages
	Psalms.Sections.__index = Psalms.Sections
	local LocalPlayer = game:GetService('Players').LocalPlayer;
	local Mouse = LocalPlayer:GetMouse();
	local TweenService = game:GetService("TweenService");

	do
		function Psalms:Connection(Signal, Callback)
			local Con = Signal:Connect(Callback)
			return Con
		end
		--
		function Psalms:Disconnect(Connection)
			Connection:Disconnect()
		end
		--
		function Psalms:Round(Number, Float)
			return Float * math.floor(Number / Float)
		end
		--
		function Psalms.NextFlag()
			Psalms.UnNamedFlags = Psalms.UnNamedFlags + 1
			return string.format("%.14g", Psalms.UnNamedFlags)
		end
		--
		function Psalms:RGBA(r, g, b, alpha)
		...
		--
		function Psalms:LoadConfig(Config)
		...
		function Psalms:SetOpen(bool)
			if typeof(bool) == 'boolean' then
				Psalms.Open = bool;
				Psalms.Holder.Visible = bool;
			end
		end;
		--
		function Psalms:IsMouseOverFrame(Frame)
		...
		--
		function Psalms:ChangeAccent(Color)
			Psalms.Accent = Color

			for obj, theme in next, Psalms.ThemeObjects do
				...
				elseif theme:IsA("ScrollingFrame") then
					theme.ScrollBarImageColor3 = Psalms.Accent
				end
			end
		end
		--
		function Psalms:Resize(object, background)
			local start, objectposition, dragging, currentpos, currentsize

			Psalms:Connection(object.MouseButton1Down, function(input)
				dragging = true
				start = input
			end)
			Psalms:Connection(Mouse.Move, function(input)
				if dragging then
					...
					for Index, Page in pairs(Psalms.Pages) do
						Page.Elements.Button.Size = UDim2.new(0, Psalms.PageAmount and ((((background.Size.X.Offset - 35) - ((Psalms.PageAmount - 1) * 2)) / Psalms.PageAmount)) - 3 or 65, 1, 0);
					end
				end;
			end)
			Psalms:Connection(game:GetService("UserInputService").InputEnded, function(input)
	...
	-- // Colorpicker Element
	do
		function Psalms:NewPicker(name, default, defaultalpha, parent, count, flag, callback)
			-- // Instances
			...
			Color.Name = "Color"
			Color.FontFace = Font.new("rbxasset://fonts/families/Gotham.json")
			Color.Text = "" 
			...
			local function update()
				...
				if flag then
					Psalms.Flags[flag] = Psalms:RGBA(hsv.r * 255, hsv.g * 255, hsv.b * 255, alpha)
				end

				callback(Psalms:RGBA(hsv.r * 255, hsv.g * 255, hsv.b * 255, alpha))
			end
			...
			Psalms:Connection(game:GetService("UserInputService").InputChanged, function(input)
				if input.UserInputType == Enum.UserInputType.Touch then
			...
			Psalms:Connection(game:GetService("UserInputService").InputBegan, function(Input)
				if ColorWindow.Visible and Input.UserInputType == Enum.UserInputType.Touch then
					if not Psalms:IsMouseOverFrame(ColorWindow) and not Psalms:IsMouseOverFrame(Icon) and not Psalms:IsMouseOverFrame(parent) then
						ColorWindow.Visible = false
						parent.ZIndex = 1
					end
				end
	...
	function Psalms:NewInstance(Inst, Theme)
		local Obj = Instance.new(Inst)
		if Theme then
			table.insert(Psalms.ThemeObjects, Obj)
			if Obj:IsA("Frame") or Obj:IsA("TextButton") then
				Obj.BackgroundColor3 = Psalms.Accent;
				if Obj:IsA("ScrollingFrame") then
					Obj.ScrollBarImageColor3 = Psalms.Accent
				end
			elseif Obj:IsA("TextLabel") or Obj:IsA("TextBox") then
				Obj.TextColor3 = Psalms.Accent;
			elseif Obj:IsA("ImageLabel") or Obj:IsA("ImageButton") then
				Obj.ImageColor3 = Psalms.Accent;
			elseif Obj:IsA("UIStroke") then
				Obj.Color = Psalms.Accent;
	...
	function Psalms:updateNotifsPositions(position)
		for i, v in pairs(Psalms.Notifs) do 
	...
	function Psalms:Notification(message, duration, color)
		local notification = {Container = nil, Objects = {}}
		--
		local Position = Vector2.new(20, 20)
		--
		local NotifContainer = Instance.new('Frame', Psalms.ScreenGUI)
		...
		Value.Name = "Value"
		Value.FontFace = Font.new("rbxasset://fonts/families/Gotham.json")
	        Value.RichText = true
		Value.Text = message
		...
		Accent.Name = "Accent"
		Accent.BackgroundColor3 = color ~= nil and color or Psalms.Accent
		...
		function notification:remove()
			table.remove(Psalms.Notifs, table.find(Psalms.Notifs, notification))
			Psalms:updateNotifsPositions(Position)
			task.wait(0.5)
		...
		table.insert(Psalms.Notifs, notification)
		NotifContainer.Position = UDim2.new(0,Position.X,0,Position.Y + (table.find(Psalms.Notifs, notification) * 25))
		Psalms:updateNotifsPositions(Position)
	...
	do
		local Pages = Psalms.Pages;
		local Sections = Psalms.Sections;
		--
		function Psalms:Window(Options)
			local Window = {
				Pages = {};
				Sections = {};
				Elements = {};
			...
			local AccentOutline = Psalms:NewInstance("TextButton", true)
			AccentOutline.Name = "AccentOutline"
			AccentOutline.AnchorPoint = Vector2.new(0,0)
			AccentOutline.BackgroundColor3 = Psalms.Accent
			...
			Title.Name = "Title"
			Title.FontFace = Font.new("rbxasset://fonts/families/Gotham.json")
			...
			-- // Dragging
			Psalms:Connection(AccentOutline.MouseButton1Down, function()
			...
			Psalms:Connection(game:GetService("UserInputService").InputBegan, function(Input)
				if Input.KeyCode == Psalms.UIKey then
					Psalms:SetOpen(not Psalms.Open)
				end
			end)
			Psalms:Resize(Resize, AccentOutline)
			...
			-- // Returns
			Psalms.Holder = AccentOutline
			Psalms.PageAmount = Window.PageAmount;
			return setmetatable(Window, Psalms)
		end;
		--
		function Psalms:Page(Properties)
			...
			local Accent = Psalms:NewInstance("Frame", true)
			Accent.Name = "Accent"
			Accent.BackgroundColor3 = Psalms.Accent
			Accent.BorderColor3 = Color3.fromRGB(0, 0, 0)
			Accent.BorderSizePixel = 0
			Accent.Size = UDim2.new(1, 0, 0, 1)
			...
			PageName.FontFace = Font.new("rbxasset://fonts/families/Gotham.json")
			...
			local WAccent = Psalms:NewInstance("Frame", true)
			WAccent.Name = "WAccent"
			WAccent.BackgroundColor3 = Psalms.Accent
			...
			--
			Psalms:Connection(NewButton.MouseButton1Down, function()
			...
			Psalms.Pages[#Psalms.Pages + 1] = Page
			Page.Window:UpdateTabs()
			return setmetatable(Page, Psalms.Pages)
		end
		--
		function Psalms:Watermark(Properties)
    local Watermark = {
        Name = (Properties.Name or Properties.name or "<font color=\"rgb(255, 255, 0)\"><b>watermark text | placeholder</b></font>"),
    }
    ...
    Outline.Parent = Psalms.ScreenGUI
    ...
    return Watermark
		...
		function Pages:Weapon(Properties)
			...
			--
			Psalms:Connection(NewWeapon.MouseButton1Down, function()
			...
			return setmetatable(Weapon, Psalms.Pages)
		end
		--
		function Pages:Section(Properties)
			...
			return setmetatable(Section, Psalms.Sections)
		end
		--
		function Sections:Toggle(Properties)
			...
			local function SetState()
				...
				Psalms.Flags[Toggle.Flag] = Toggle.Toggled
				Toggle.Callback(Toggle.Toggled)
			end
			--
			Psalms:Connection(NewToggle.MouseButton1Down, SetState)
			...
			function Toggle:Keybind(Properties)
					...
					if string.find(tostring(newkey), "Enum") then
						if c then
							...
							if Keybind.Flag then
								Psalms.Flags[Keybind.Flag] = false
							end
						...
						elseif newkey ~= nil then
							...
							local text = (Psalms.Keys[newkey] or tostring(newkey):gsub("Enum.KeyCode.", ""))

							Value.Text = text
						end

						Psalms.Flags[Keybind.Flag .. "_KEY"] = newkey
					elseif table.find({ "Always", "Toggle", "Hold" }, newkey) then
						if not Keybind.UseKey then
							Psalms.Flags[Keybind.Flag .. "_KEY STATE"] = newkey
							Keybind.Mode = newkey
							if Keybind.Mode == "Always" then
								State = true
								if Keybind.Flag then
									Psalms.Flags[Keybind.Flag] = State
								end
						...
						State = newkey
						if Keybind.Flag then
							Psalms.Flags[Keybind.Flag] = newkey
						end
				...
				KInline.MouseButton1Click:Connect(function()
					if not Keybind.Binding then
						...
						Keybind.Binding = Psalms:Connection(
							...
							function(input, gpe)
								...
								Psalms:Disconnect(Keybind.Binding)
								task.wait()
								Keybind.Binding = nil
							end
						)
				...
				--
				Psalms:Connection(game:GetService("UserInputService").InputBegan, function(inp)
					if (inp.KeyCode == Key or inp.UserInputType == Key) and not Keybind.Binding and not Keybind.UseKey then
						if Keybind.Mode == "Hold" then
							if Keybind.Flag then
								Psalms.Flags[Keybind.Flag] = true
							end
							c = Psalms:Connection(game:GetService("RunService").RenderStepped, function()
				...
				--
				Psalms:Connection(game:GetService("UserInputService").InputEnded, function(inp)
				...
				--
				Psalms:Connection(KInline.MouseButton2Down, function()
					ModeBox.Visible = true
					NewToggle.ZIndex = 5
				end)
				--
				Psalms:Connection(Hold.MouseButton1Down, function()
					set("Hold")
				...
				--
				Psalms:Connection(Toggle.MouseButton1Down, function()
					set("Toggle")
				...
				--
				Psalms:Connection(Always.MouseButton1Down, function()
					set("Always")
					Hold.TextColor3 = Color3.fromRGB(145,145,145)
					Toggle.TextColor3 = Color3.fromRGB(145,145,145)
				...
				Psalms:Connection(game:GetService("UserInputService").InputBegan, function(Input)
					if ModeBox.Visible and Input.UserInputType == Enum.UserInputType.Touch then
						if not Psalms:IsMouseOverFrame(ModeBox) then
							ModeBox.Visible = false
							NewToggle.ZIndex = 1
						end
					end
				...
				--
				Psalms.Flags[Keybind.Flag .. "_KEY"] = Keybind.State
				Psalms.Flags[Keybind.Flag .. "_KEY STATE"] = Keybind.Mode
			...
			function Toggle:Colorpicker(Properties)
				...
				local colorpickertypes = Psalms:NewPicker(
					"",
					Colorpicker.State,
					Colorpicker.Alpha,
					NewToggle,
			...
			Psalms.Flags[Toggle.Flag] = Toggle.State
		...
		function Sections:Slider(Properties)
			...
			local Accent = Psalms:NewInstance("TextButton", true)
			Accent.Name = "Accent"
			Accent.BackgroundColor3 = Psalms.Accent
			Accent.BorderColor3 = Color3.fromRGB(0, 0, 0)
			Accent.BorderSizePixel = 0
			...
			local function Set(value)
				value = math.clamp(Psalms:Round(value, Slider.Decimals), Slider.Min, Slider.Max)
				...
				Psalms.Flags[Slider.Flag] = value
			...
			--
			Psalms:Connection(Inline.InputBegan, function(input)
				if input.UserInputType == Enum.UserInputType.Touch then
					Sliding = true
					ISlide(input)
				end
			end)
			Psalms:Connection(Inline.InputEnded, function(input)
				if input.UserInputType == Enum.UserInputType.Touch then
					Sliding = false
				end
			end)
			Psalms:Connection(Accent.InputBegan, function(input)
				if input.UserInputType == Enum.UserInputType.Touch then
					Sliding = true
					ISlide(input)
				end
			end)
			Psalms:Connection(Accent.InputEnded, function(input)
				if input.UserInputType == Enum.UserInputType.Touch then
					Sliding = false
				end
			end)
			...
			Psalms.Flags[Slider.Flag] = Slider.State
			Set(Slider.State)
		...
		function Sections:List(Properties)
			...
			Icon.Name = "Icon"
			Icon.FontFace = Font.new("rbxasset://fonts/families/Gotham.json")
			Icon.Text = "+"
			...
			-- // Connections
			Psalms:Connection(Inline.MouseButton1Down, function()
			...
			Psalms:Connection(game:GetService("UserInputService").InputBegan, function(Input)
				if ContentOutline.Visible and Input.UserInputType == Enum.UserInputType.Touch then
					if not Psalms:IsMouseOverFrame(ContentOutline) and not Psalms:IsMouseOverFrame(Inline) then
						ContentOutline.Visible = false
						NewList.ZIndex = 1
						Icon.Text = "+"
					end
				...
				button.MouseButton1Down:Connect(function()
					if Dropdown.Max then
						if table.find(chosen, option) then
							...
							Psalms.Flags[Dropdown.Flag] = chosen
						...
						NewList.ZIndex = 1
						Icon.Text = "+"
						Psalms.Flags[Dropdown.Flag] = option
			...
			local function createoptions(tbl)
				for _, option in next, tbl do
					...
					OptionLabel.FontFace = Font.new("rbxasset://fonts/families/Gotham.json")
    ...
    for _, opt in next, Dropdown.OptionInsts do
        if opt.button and opt.button:IsA("Instance") then
            opt.button:Destroy()
        end
    end
    ...
    Dropdown.Callback(chosen)
		...
		function Sections:ListBox(Properties)
			...
			local ContentInline = Psalms:NewInstance("ScrollingFrame", true)
			ContentInline.Name = "ContentInline"
			...
			ContentInline.ScrollBarImageColor3 = Psalms.Accent
    ...
    Dropdown.Callback(chosen)
		...
		function Sections:Colorpicker(Properties)
			...
			function Colorpicker:Colorpicker(Properties)
				...
				-- // Functions
				Colorpicker.Colorpickers = Colorpicker.Colorpickers + 1
				local Newcolorpickertypes = Psalms:NewPicker(
					"",
		...
		function Sections:Keybind(Properties)
			...
			--
			Psalms:Connection(Inline.MouseButton2Down, function()
				ModeBox.Visible = true
				NewBind.ZIndex = 5
			end)
			--
		...
		function Sections:Textbox(Properties)
			...
			Value.FocusLost:Connect(function()
				...
				Psalms.Flags[Textbox.Flag] = Value.Text
			end)
			--
			local function set(str)
				Value.Text = str
				Psalms.Flags[Textbox.Flag] = str
		...
		function Sections:Button(Properties)
			...
			Psalms:Connection(Inline.MouseButton1Down, function()
				Button.Callback()
				Value.TextColor3 = Psalms.Accent
				task.wait(0.1)
				Value.TextColor3 = Color3.fromRGB(255,255,255)
			end)
			...
			function Button:Button(Properties)
				...
				iValue.Name = "Value"
				iValue.FontFace = Font.new("rbxasset://fonts/families/Gotham.json")
				...
				--
				Psalms:Connection(iInline.MouseButton1Down, function()
					InButton.Callback()
					iValue.TextColor3 = Psalms.Accent
					task.wait(0.1)
					iValue.TextColor3 = Color3.fromRGB(255,255,255)
				end)
			end
		...
		function Pages:PlayerList(Properties)
			...
			SectionName.FontFace = Font.new("rbxasset://fonts/families/Gotham.json")
			SectionName.Text = "Player List"
			SectionName.TextColor3 = Color3.fromRGB(255, 255, 255)
			SectionName.TextSize = Psalms.FSize
			...
			local List = Psalms:NewInstance("ScrollingFrame", true)
			List.Name = "List"
			List.AutomaticCanvasSize = Enum.AutomaticSize.Y
			List.BottomImage = "rbxassetid://7783554086"
			List.CanvasSize = UDim2.new()
			List.MidImage = "rbxassetid://7783554086"
			List.ScrollBarImageColor3 = Psalms.Accent
			...
			PlayerName1.FontFace = Font.new("rbxasset://fonts/families/Gotham.json")
			PlayerName1.Text = "Select a Player."
			PlayerName1.TextColor3 = Color3.fromRGB(255, 255, 255)
			PlayerName1.TextSize = Psalms.FSize
			...
			PriorityLabel.FontFace = Font.new("rbxasset://fonts/families/Gotham.json")
			PriorityLabel.Text = "Prioritize"
			PriorityLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
			PriorityLabel.TextSize = Psalms.FSize
			...
			FriendLabel.FontFace = Font.new("rbxasset://fonts/families/Gotham.json")
			FriendLabel.Text = "Friendly"
			FriendLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
			FriendLabel.TextSize = Psalms.FSize
			...
			local function handleoptionclick(option, button, accent)
				button.MouseButton1Click:Connect(function()
					chosen = option
					Psalms.Flags[Playerlist.Flag] = option
			...
			local function createoptions(tbl)
				for i, option in next, tbl do
					...
					PlayerName.FontFace = Font.new("rbxasset://fonts/families/Gotham.json")
					PlayerName.Text = option.Name
					PlayerName.TextColor3 = Color3.fromRGB(255, 255, 255)
					PlayerName.TextSize = Psalms.FSize
					...
					PlayerStatus.FontFace = Font.new("rbxasset://fonts/families/Gotham.json")
					PlayerStatus.Text = option == game.Players.LocalPlayer and "Local Player" or table.find(Psalms.Friends, option) and "Friendly" or table.find(Psalms.Priorities, option) and "Priority" or "None"
					PlayerStatus.TextColor3 = option == game.Players.LocalPlayer and Color3.fromRGB(0, 170, 255) or table.find(Psalms.Friends, option) and Color3.fromRGB(0,255,0) or table.find(Psalms.Priorities, option) and Color3.fromRGB(255,0,0) or Color3.fromRGB(255,255,255)
					PlayerStatus.TextSize = Psalms.FSize
					...
					local AccentLine = Psalms:NewInstance("Frame", true)
					AccentLine.Name = "AccentLine"
					AccentLine.BackgroundColor3 = Psalms.Accent
			...
			function Playerlist:Refresh(tbl, dontchange)
				...
				Psalms.Flags[Playerlist.Flag] = chosen
			end
			--
			Priority.MouseButton1Click:Connect(function()
				if Playerlist.CurrentPlayer ~= nil and table.find(Psalms.Friends, Playerlist.CurrentPlayer) then
					table.remove(Psalms.Friends, table.find(Psalms.Friends, Playerlist.CurrentPlayer))
				end
				if Playerlist.CurrentPlayer ~= nil and not table.find(Psalms.Priorities, Playerlist.CurrentPlayer) then
					table.insert(Psalms.Priorities, Playerlist.CurrentPlayer)
				...
				elseif Playerlist.CurrentPlayer ~= nil and table.find(Psalms.Priorities, Playerlist.CurrentPlayer) then
					table.remove(Psalms.Priorities, table.find(Psalms.Priorities, Playerlist.CurrentPlayer))
			...
			Friend.MouseButton1Click:Connect(function()
				if Playerlist.CurrentPlayer ~= nil and table.find(Psalms.Priorities, Playerlist.CurrentPlayer) then
					table.remove(Psalms.Priorities, table.find(Psalms.Priorities, Playerlist.CurrentPlayer))
				end
				if Playerlist.CurrentPlayer ~= nil and not table.find(Psalms.Friends, Playerlist.CurrentPlayer) then
					table.insert(Psalms.Friends, Playerlist.CurrentPlayer)
				...
				elseif Playerlist.CurrentPlayer ~= nil and table.find(Psalms.Friends, Playerlist.CurrentPlayer) then
					table.remove(Psalms.Friends, table.find(Psalms.Friends, Playerlist.CurrentPlayer))
					optioninstances[Playerlist.CurrentPlayer].status.Text = "None"
					optioninstances[Playerlist.CurrentPlayer].status.TextColor3 = Color3.fromRGB(255,255,255)
				end
		...
		--
		function Psalms:Indicator(Properties)
			local Indicator = {
			...
			-- // Dragging
			Psalms:Connection(Outline.MouseButton1Down, function()
				local Location = game:GetService("UserInputService"):GetMouseLocation()
			...
			function Indicator:NewValue(Properties)
				...
				IndValue.FontFace = Font.new("rbxasset://fonts/families/Gotham.json")
			...
			function Indicator:NewBar(Properties)
				...
				Title1.Name = "Title"
				Title1.FontFace = Font.new("rbxasset://fonts/families/Gotham.json")
				...
				IndValue1.FontFace = Font.new("rbxasset://fonts/families/Gotham.json")
				IndValue1.Text = "0"
				...
				local Bar = Psalms:NewInstance("Frame", true)
				Bar.Name = "Bar"
				Bar.BackgroundColor3 = Psalms.Accent
				...
				function NewBarInd:UpdateValue(NewValue)
					NewValue = math.clamp(Psalms:Round(NewValue, 1), NewBarInd.Min, NewBarInd.Max)
...
-- Local Variables
local Flags = Psalms.Flags
local Pointers = Psalms.Pointers
local Utility = Psalms.Utility

getfenv(0)["Psalms"] = Psalms;
